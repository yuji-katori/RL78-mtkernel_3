...
#ifdef CLANGSPEC
			knl_strncpy(mbxcb->name, pk_cmbx->dsname,		
#else
 ...
 */
#ifdef CLANGSPEC
SYSCALL ER tk_snd_mbx( ID mbxid, void *pk_msg )
#else
 ...
 */
#ifdef CLANGSPEC
SYSCALL ER tk_rcv_mbx( ID mbxid, void *ppk_msg, TMO tmout )
#else
...
#endif /* CLANGSPEC */
{
	...
	if ( headmsg(mbxcb) != NULL ) {
...
#ifdef CLANGSPEC
		*(T_MSG**)ppk_msg = headmsg(mbxcb);
		headmsg(mbxcb) = nextmsg(*(T_MSG**)ppk_msg);
#else
		*ppk_msg = headmsg(mbxcb);
		...
		knl_ctxtsk->wspec = ( (mbxcb->mbxatr & TA_TPRI) != 0 ) ? &knl_wspec_mbx_tpri : &knl_wspec_mbx_tfifo;
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_mailbox_getname(ID id, VB **name)
#else
