...
#ifdef CLANGSPEC
		knl_strncpy(mpfcb->name, pk_cmpf->dsname, OBJECT_NAME_LENGTH);
#else
...
wait_mpf:
	...
	knl_ctxtsk->wspec = ( (mpfcb->mpfatr & TA_TPRI) != 0 ) ? &knl_wspec_mpf_tpri : &knl_wspec_mpf_tfifo;
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_fix_memorypool_getname(ID id, VB **name)
#else
