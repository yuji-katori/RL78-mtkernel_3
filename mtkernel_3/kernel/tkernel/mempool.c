 ...
 */
LOCAL void *knl_get_blk( MPLCB *mplcb, SZ blksz )
{
	QUEUE	*q, *aq, *aq2;
	IMACB*	imacb = (IMACB*)&(mplcb->mplsz);
	...
	knl_removeFreeQue(q);
	aq = (q == mplcb->mempool) ? &(mplcb->areaque) :  q - 1;

	/* If there is a fragment smaller than the minimum fragment size, allocate them together */
	if ( FreeSize(q) - blksz >= MIN_FRAGMENT + sizeof(QUEUE) ) {
...
#ifdef CLANGSPEC
			knl_strncpy(mplcb->name, pk_cmpl->dsname, OBJECT_NAME_LENGTH);
#else
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_memorypool_getname(ID id, VB **name)
#else
