 ...
 */
#ifdef CLANGSPEC
EXPORT DevCB* knl_searchDevCB( CONST VB *devnm )
#else
...
#ifdef CLANGSPEC
		if ( devcb->devnm[0] == devnm[0] && knl_strcmp(devcb->devnm, devnm) == 0 ) {
#else
 ...
 */
#ifdef CLANGSPEC
LOCAL DevCB* newDevCB( CONST VB *devnm )
#else
...
#ifdef CLANGSPEC
	knl_strncpy(devcb->devnm, devnm, L_DEVNM+1);
#else
 ...
 */
#ifdef CLANGSPEC
SYSCALL ID tk_def_dev( CONST VB *devnm, CONST T_DDEV *pk_ddev, T_IDEV *pk_idev )
#else
 ...
 */
#ifdef CLANGSPEC
EXPORT INT knl_phydevnm( VB *pdevnm, CONST VB *ldevnm )
{
	VB	c;
#else
 ...
 */
#ifdef CLANGSPEC
LOCAL void logdevnm( VB *ldevnm, VB *pdevnm, INT unitno )
{
	VB	unostr[12], *cp;

	knl_strcpy(ldevnm, pdevnm);
#else
 ...
 */
#ifdef CLANGSPEC
SYSCALL ID tk_get_dev( ID devid, VB *devnm )
#else
 ...
 */
#ifdef CLANGSPEC
SYSCALL ID tk_ref_dev( CONST VB *devnm, T_RDEV *pk_rdev )
{
	VB	pdevnm[L_DEVNM + 1];
#else
...
#ifdef CLANGSPEC
			knl_strncpy(pk_ldev->devnm, devcb->devnm, L_DEVNM);
#else
...
#ifdef CLANGSPEC
	knl_strncpy((char*)&cmbf.exinf, OBJNAME_DMMBF, sizeof(cmbf.exinf));
#else
...
#ifdef CLANGSPEC
	ercd = CreateMLock(&knl_DevMgrLock, OBJNAME_DMLOCK);
#else
