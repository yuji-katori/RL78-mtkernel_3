...
#else
	CallUserHandlerP1(cyccb->exinf, (void (*)(void *))cyccb->cychdr, cyccb);
#endif /* CLANGSPEC */
	DISABLE_INTERRUPT;
}
...
#ifdef CLANGSPEC
		knl_strncpy(cyccb->name, pk_ccyc->dsname, OBJECT_NAME_LENGTH);
#else
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_cyclichandler_getname(ID id, VB **name)
#else
...
#ifdef CLANGSPEC
		knl_strncpy(almcb->name, pk_calm->dsname, OBJECT_NAME_LENGTH);
#else
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_alarmhandler_getname(ID id, VB **name)
#else
