...
LOCAL void knl_msg_to_mbf( MBFCB *mbfcb, CONST void *msg, INT msgsz )
{
	...
	*(HEADER*)&buffer[tail] = msgsz;
	...
	if ( (remsz = mbfcb->bufsz - tail) < msgsz ) {
		knl_memcpy(&buffer[tail], msg, remsz);
		msg = (VB*)msg + remsz;
		msgsz -= remsz;
		tail = 0;
	}
	knl_memcpy(&buffer[tail], msg, msgsz);
	tail += ROUNDSZ(msgsz);
	if ( tail >= mbfcb->bufsz ) {
		tail = 0;
	}
...
SYSCALL ID tk_cre_mbf( CONST T_CMBF *pk_cmbf )
{
	...
	if ( bufsz > 0 ) {
		...
		if ( (pk_cmbf->mbfatr & TA_USERBUF) != 0 ) {
			...
			/* Use user buffer */
			msgbuf = pk_cmbf->bufptr;
		} else {
			/* Allocate by kernel */
			msgbuf = knl_Imalloc(bufsz);
			if ( msgbuf == NULL ) {
				return E_NOMEM;
			}
		}
...
#ifdef CLANGSPEC
			knl_strncpy(mbfcb->name, pk_cmbf->dsname, OBJECT_NAME_LENGTH);
#else
			knl_strncpy((char*)mbfcb->name, (char*)pk_cmbf->dsname, OBJECT_NAME_LENGTH);
#endif /* CLANGSPEC */
		}
#endif
		ercd = mbfid;
...
SYSCALL ER tk_snd_mbf( ID mbfid, CONST void *msg, INT msgsz, TMO tmout )
{
	...
	if ( !isQueEmpty(&mbfcb->recv_queue) ) {
		...
		knl_memcpy(tcb->winfo.rmbf.msg, msg, msgsz);
...
LOCAL INT knl_mbf_to_msg( MBFCB *mbfcb, void *msg )
{
	...
	if ( (remsz = mbfcb->bufsz - head) < msgsz ) {
		knl_memcpy(msg, &buffer[head], remsz);
		msg = (VB*)msg + remsz;
		msgsz -= remsz;
		head = 0;
	}
	knl_memcpy(msg, &buffer[head], msgsz);
	head += ROUNDSZ(msgsz);
	if ( head >= mbfcb->bufsz ) {
		head = 0;
	}
...
SYSCALL INT tk_rcv_mbf( ID mbfid, void *msg, TMO tmout )
{
	...
	if ( !knl_mbf_empty(mbfcb) ) {
		...
		knl_memcpy(msg, tcb->winfo.smbf.msg, rcvsz);
		knl_wait_release_ok(tcb);
		knl_mbf_wakeup(mbfcb);
	} else {
		ercd = E_TMOUT;
...
SYSCALL ER tk_ref_mbf( ID mbfid, T_RMBF *pk_rmbf )
{
	...
	if ( mbfcb->mbfid == 0 ) {
		...
		if ( !knl_mbf_empty(mbfcb) ) {
			pk_rmbf->msgsz = *(HEADER*)&((VB*)mbfcb->buffer)[mbfcb->head];
		} else {
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_messagebuffer_getname(ID id, VB **name)
#else
