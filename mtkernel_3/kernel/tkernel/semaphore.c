...
#ifdef CLANGSPEC
			knl_strncpy(semcb->name, pk_csem->dsname, OBJECT_NAME_LENGTH);
#else
			knl_strncpy((char*)semcb->name, (char*)pk_csem->dsname, OBJECT_NAME_LENGTH);
#endif /* CLANGSPEC */
		}
#endif
		ercd = semid;
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_semaphore_getname(ID id, VB **name)
#else
