...
SYSCALL ID tk_cre_tsk( CONST T_CTSK *pk_ctsk )
{
	...
	if ( (pk_ctsk->tskatr & TA_USERBUF) != 0 ) {
		...
		sstksz  = (sstksz  + 1) >> 1 << 1;	/* Align to a multiple of 2 */
		stack = knl_Imalloc(sstksz);
		if ( stack == NULL ) {
			return E_NOMEM;
		}
#endif
...
#ifdef CLANGSPEC
		knl_strncpy(tcb->name, pk_ctsk->dsname, OBJECT_NAME_LENGTH);
#else
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_task_getname(ID id, VB **name)
#else
