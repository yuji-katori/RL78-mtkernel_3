...
#ifdef CLANGSPEC
			knl_strncpy(mtxcb->name, pk_cmtx->dsname, OBJECT_NAME_LENGTH);
#else
			knl_strncpy((char*)mtxcb->name, (char*)pk_cmtx->dsname, (UINT)OBJECT_NAME_LENGTH);
#endif /* CLANGSPEC */
		}
#endif
		ercd = mtxid;
...
SYSCALL ER tk_ref_mtx( ID mtxid, T_RMTX *pk_rmtx )
{
	...
	if ( mtxcb->mtxid == 0 ) {
		...
		pk_rmtx->htsk = ( mtxcb->mtxtsk != NULL ) ? mtxcb->mtxtsk->tskid : 0;
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_mutex_getname(ID id, VB **name)
#else
