...
#ifdef CLANGSPEC
			knl_strncpy(flgcb->name, pk_cflg->dsname, OBJECT_NAME_LENGTH);
#else
			knl_strncpy((char*)flgcb->name, (char*)pk_cflg->dsname, OBJECT_NAME_LENGTH);
#endif /* CLANGSPEC */
		}
#endif
		ercd = flgid;
 ...
 */
#ifdef CLANGSPEC
EXPORT ER knl_eventflag_getname(ID id, VB **name)
#else
