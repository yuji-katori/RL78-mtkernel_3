...
typedef struct td_rmpf {
	void	*exinf;		/* Extended information */
	ID	wtsk;		/* Wait task ID */
	SZ	frbcnt;		/* Number of free blocks */
} TD_RMPF;
...
/* Object name */
#ifdef CLANGSPEC
IMPORT ER td_ref_dsname( UINT type, ID id, VB *dsname );
IMPORT ER td_set_dsname( UINT type, ID id, CONST VB *dsname );
#else
