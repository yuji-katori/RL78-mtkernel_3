...
typedef struct t_rsys {
	...
	ID	schedtskid;	/* ID of the task that should be the execution state */
} T_RSYS;
...
typedef struct t_rdev {
	...
	SZ	blksz;		/* Specific data block size (-1: Unknown) */
...
typedef struct t_devreq {
	...
	SZ	size;		/* I:Request size */
	void	*buf;		/* I:Input/output buffer address */
	SZ	asize;		/* O:Result size */
